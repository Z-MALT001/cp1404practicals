...
def is_long_word(word, length=5):
    ...
    True
    """
    return len(word) >= length # WOOOAH
    # return f"{len(word) >= length}"
...
def format_phrase_as_sentence(sentence):
    """
     Format a phrase as a sentence, with a capital at the start and end with a fill stop.
    >>> format_phrase_as_sentence('hello')
    'Hello.'
    >>> format_phrase_as_sentence('It is an ex parrot.')
    'It is an ex parrot.'
    >>> format_phrase_as_sentence('Nuh uh.')
    'Nuh uh.'
    """
    if sentence[-1] != ".":
        sentence += "."
